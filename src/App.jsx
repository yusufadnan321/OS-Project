...
function simulate(processList) {
  ...
  const maxTime = totalBurst + lastArrival + 1000
  ...
  const pushExec = (pid, name, type, t) => {
    if (timeline.length === 0) {
      ...
      if (last.pid === pid) last.end = t + 1
      else timeline.push({ pid, name, type, start: t, end: t + 1 })
    }
  }
...
function colorFor(type) {
  return type === 'foreground' ? 'bg-blue-500' : 'bg-green-600'
}
      ...
      <div className="w-full bg-white p-3 rounded shadow">
        <div className="flex items-start gap-2">
          {timeline.map((seg, idx) => {
            ...
            return (
              <div key={idx} className={`flex items-center justify-center text-white text-sm ${colorFor(seg.type)}`} style={{ width: width + 'px', height: '40px' }} title={`${seg.name} (${seg.start}-${seg.end})`}>
                {seg.name}
              </div>
            )
          })}
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <header className="mb-6">
        ...
        <p className="text-sm text-gray-600">Two queues: Foreground (Round Robin, quantum = 4ms) and Background (FCFS). Foreground processes have  priority.</p>
      </header>
